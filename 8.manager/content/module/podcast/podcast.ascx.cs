...
public partial class content_module_podcast_podcast : SmartUserControl
{
	#region [ Properties ]
	...
	/// 
	/// </summary>
	protected PodcastCategoria entidadePodcastCategoria;
	/// <summary>
	...
	/// 
	/// </summary>
	protected PodcastCategoriaService podcastCategoriaService;
	/// <summary>
	...
	protected void LoadForm()
	{
			...
			&& conteudo.Podcast.PodcastIdiomas.Count > 0)
		{
			...
			if (entidadePodcast.PodcastCategoria != null && entidadePodcast.PodcastCategoria.PodcastCategoriaId > 0)
			{
				ddlCategoria.SelectedValue = entidadePodcast.PodcastCategoria.PodcastCategoriaId.ToString();
			}

			//Arquivo
			if (entidadePodcast.DataHoraPublicacao.ToString() != "")
			{
				txtdtPublicacao.Text = entidadePodcast.DataHoraPublicacao.ToString("dd/MM/yyyy");
			}
			else
			{
	...
	protected void FormToLoad()
	{
		...
		conteudo.ConteudoTipo.ConteudoTipoId = 5;
		...
		if (!String.IsNullOrEmpty(ddlCategoria.SelectedValue))
		{
			...
			entidadePodcast.PodcastCategoria.PodcastCategoriaId = Convert.ToInt32(ddlCategoria.SelectedValue);
		}
	...
	{
		entidadePodcastCategoria = new PodcastCategoria();

		podcastCategoriaService = new PodcastCategoriaService();

		IList<PodcastCategoria> entidadePodcastCategoriaList = new List<PodcastCategoria>();

		entidadePodcastCategoriaList = podcastCategoriaService.RetornaTodos();

		ddlCategoria.Items.Clear();
		ddlCategoria.Items.Add(new ListItem("Selecione", ""));

		foreach (PodcastCategoria item in entidadePodcastCategoriaList)
		{
			ddlCategoria.Items.Add(new ListItem(item.PodcastCategoriaIdiomas[0].Descricao, item.PodcastCategoriaId.ToString()));
		}
