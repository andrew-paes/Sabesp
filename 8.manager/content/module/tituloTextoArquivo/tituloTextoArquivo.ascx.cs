...
public partial class content_module_tituloTextoArquivo_tituloTextoArquivo : SmartUserControl
{
	#region [ Properties ]
	...
	private void InitGridView()
	{
		...
		this.DT.Columns.Add(new DataColumn("Arquivo", typeof(string)));
	...
	private void AddRow()
	{
		if (!String.IsNullOrEmpty(txtTexto.Text))
		{
			...
			drow["Arquivo"] = uplField.FileName;
	...
	private void AddRow(ControleIdiomaDado entidadeParam)
	{
		...
		drow["Arquivo"] = entidadeParam.ArquivoNome;
		drow["idGridView"] = ++this.RowsCount;
		this.DT.Rows.Add(drow);
	}
		...
		for (int i = 0; i < this.DT.Rows.Count; i++)
		{
			if (Convert.ToString(this.DT.Rows[i]["idGridView"]).Equals(hddIdGridView.Value))
			{
				if (!String.IsNullOrEmpty(txtTexto.Text))
				{
					...
					this.DT.Rows[i]["Arquivo"] = uplField.FileName;
	...
	protected void gdvTituloTextoLink_SelectedIndexChanged(object sender, EventArgs e)
	{
		...
		uplField.FileName = gdvTituloTextoLink.SelectedDataKey["Arquivo"].ToString();
	}
	...
	private void FormToLoad()
	{
		...
		if (IdRegistro > 0)
		{
			...
			for (int i = 0; i < this.DT.Rows.Count; i++)
			{
				...
				if (!String.IsNullOrEmpty(this.DT.Rows[i]["Arquivo"].ToString()))
				{
					string strArquivo = this.DT.Rows[i]["Arquivo"].ToString();

					entidadeControleIdiomaDado.ArquivoNome = strArquivo;
					entidadeControleIdiomaDado.ArquivoTamanho = 0;
					entidadeControleIdiomaDado.ArquivoExtensao = strArquivo.Substring(strArquivo.LastIndexOf("."), strArquivo.Length - strArquivo.LastIndexOf("."));
				}
